...
export default function LocationTaskInterface() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      // Try with relation first; gracefully fall back if relation/table not available
      ...
      let rows = data;
      if (error) {
        // Fallback: fetch tasks without relation to avoid breaking the page
        ...
        rows = (fallback.data || []).map((t: any) => ({ ...t, task_locations: [] }));
      }

      const tasksWithLocation = (rows || []).map(task => ({
        ...task,
      ...
      // Avoid noisy toasts on this page; keep UI usable if fetch partially fails
    }
  };
